	...
	StreetMapCategory.AddCustomRow(FText::GetEmpty(), false)
		[
			SAssignNew(TempHorizontalBox, SHorizontalBox)
				...
				[
					SNew(SButton)
						.ToolTipText(LOCTEXT("CreateDataTableAsset_Tooltip", "Create a new DataTable Asset from selected StreetMapComponent."))
						.OnClicked(this, &FStreetMapComponentDetails::OnCreateDataTableAssetClicked)
						.IsEnabled(bCanCreateMeshAsset)
						.HAlign(HAlign_Center)
						[
							SNew(STextBlock)
								.Text(LOCTEXT("CreateDataTableAsset", "Create DataTable Asset"))
								.Font(IDetailLayoutBuilder::GetDetailFont())
						]
				]
		];
...
FReply FStreetMapComponentDetails::OnCreateDataTableAssetClicked()
{
	if (SelectedStreetMapComponent && SelectedStreetMapComponent->GetStreetMap())
	{
		// Create Buildings DataTable
		FString StreetMapAssetPath = SelectedStreetMapComponent->StreetMap->GetPathName();
		int32 DotPosition = StreetMapAssetPath.Find(TEXT("."), ESearchCase::CaseSensitive, ESearchDir::FromEnd);
		if (DotPosition != INDEX_NONE)
		{
			StreetMapAssetPath = StreetMapAssetPath.Left(DotPosition);
		}

		FString StreetMapAssetName = SelectedStreetMapComponent->GetStreetMapAssetName();

		{
			FString PackageName = StreetMapAssetPath + TEXT("_Buildings");
			FString Name;
			FAssetToolsModule& AssetToolsModule = FModuleManager::LoadModuleChecked<FAssetToolsModule>("AssetTools");
			AssetToolsModule.Get().CreateUniqueAssetName(PackageName, TEXT("_DT"), PackageName, Name);

			TSharedPtr<SDlgPickAssetPath> PickAssetPathWidget =
				SNew(SDlgPickAssetPath)
				.Title(LOCTEXT("CreateBuildingsDataTable", "Choose Buildings DataTable Location"))
				.DefaultAssetPath(FText::FromString(PackageName));

			if (PickAssetPathWidget->ShowModal() == EAppReturnType::Ok)
			{
				PackageName = PickAssetPathWidget->GetFullAssetPath().ToString();
				Name = FPackageName::GetLongPackageAssetName(PackageName);

				UPackage* Package = CreatePackage(*PackageName);
				check(Package);

				UDataTable* BuildingsDataTable = NewObject<UDataTable>(Package, *Name, RF_Public | RF_Standalone);
				BuildingsDataTable->RowStruct = FStreetMapBuilding::StaticStruct();

				const TArray<FStreetMapBuilding>& Buildings = SelectedStreetMapComponent->GetStreetMap()->GetBuildings();
				for (int32 i = 0; i < Buildings.Num(); ++i)
				{
					const FStreetMapBuilding& Building = Buildings[i];
					FName RowName = *FString::Printf(TEXT("Building_%d"), i);

					if (!Building.BuildingName.IsEmpty())
					{
						FString SafeName = Building.BuildingName;
						SafeName.ReplaceInline(TEXT(" "), TEXT("_"));
						SafeName.ReplaceInline(TEXT("."), TEXT("_"));
						SafeName.ReplaceInline(TEXT(","), TEXT("_"));
						RowName = *SafeName;
					}
					BuildingsDataTable->AddRow(RowName, Building);
				}

				BuildingsDataTable->MarkPackageDirty();
				FAssetRegistryModule::AssetCreated(BuildingsDataTable);

				if (GIsEditor)
				{
					FNotificationInfo Info(FText::Format(LOCTEXT("BuildingsDataTableCreated", "Created Buildings DataTable: {0}"), FText::FromString(BuildingsDataTable->GetName())));
					Info.ExpireDuration = 8.0f;
					Info.bUseLargeFont = false;

					UAssetEditorSubsystem* AssetEditorSubsystem = GEditor->GetEditorSubsystem<UAssetEditorSubsystem>();
					Info.Hyperlink = FSimpleDelegate::CreateLambda([=]() { AssetEditorSubsystem->OpenEditorForAssets(TArray<UObject*>({ BuildingsDataTable })); });

					Info.HyperlinkText = FText::Format(LOCTEXT("OpenNewDataTableHyperlink", "Open {0}"), FText::FromString(BuildingsDataTable->GetName()));
		...
		// Create Roads DataTable
		{
			FString PackageName = StreetMapAssetPath + TEXT("_Roads");
			FString Name;
			FAssetToolsModule& AssetToolsModule = FModuleManager::LoadModuleChecked<FAssetToolsModule>("AssetTools");
			AssetToolsModule.Get().CreateUniqueAssetName(PackageName, TEXT("_DT"), PackageName, Name);
			...
			TSharedPtr<SDlgPickAssetPath> PickAssetPathWidget =
				SNew(SDlgPickAssetPath)
				.Title(LOCTEXT("CreateRoadsDataTable", "Choose Roads DataTable Location"))
				.DefaultAssetPath(FText::FromString(PackageName));
			...
			if (PickAssetPathWidget->ShowModal() == EAppReturnType::Ok)
			{
				...
				UDataTable* RoadsDataTable = NewObject<UDataTable>(Package, *Name, RF_Public | RF_Standalone);
				RoadsDataTable->RowStruct = FStreetMapRoad::StaticStruct();

				const TArray<FStreetMapRoad>& Roads = SelectedStreetMapComponent->GetStreetMap()->GetRoads();
				for (int32 i = 0; i < Roads.Num(); ++i)
				{
					const FStreetMapRoad& Road = Roads[i];
					FName RowName = *FString::Printf(TEXT("Road_%d"), i);

					if (!Road.RoadName.IsEmpty())
					{
						FString SafeName = Road.RoadName;
						SafeName.ReplaceInline(TEXT(" "), TEXT("_"));
						SafeName.ReplaceInline(TEXT("."), TEXT("_"));
						SafeName.ReplaceInline(TEXT(","), TEXT("_"));
						RowName = *SafeName;
					...
					RoadsDataTable->AddRow(RowName, Road);
				}

				RoadsDataTable->MarkPackageDirty();
				FAssetRegistryModule::AssetCreated(RoadsDataTable);

				if (GIsEditor)
				{
					FNotificationInfo Info(FText::Format(LOCTEXT("RoadsDataTableCreated", "Created Roads DataTable: {0}"), FText::FromString(RoadsDataTable->GetName())));
					Info.ExpireDuration = 8.0f;
					Info.bUseLargeFont = false;

					UAssetEditorSubsystem* AssetEditorSubsystem = GEditor->GetEditorSubsystem<UAssetEditorSubsystem>();
					Info.Hyperlink = FSimpleDelegate::CreateLambda([=]() { AssetEditorSubsystem->OpenEditorForAssets(TArray<UObject*>({ RoadsDataTable })); });

					Info.HyperlinkText = FText::Format(LOCTEXT("OpenNewDataTableHyperlink", "Open {0}"), FText::FromString(RoadsDataTable->GetName()));
					TSharedPtr<SNotificationItem> Notification = FSlateNotificationManager::Get().AddNotification(Info);
					if (Notification.IsValid())
					{
						Notification->SetCompletionState(SNotificationItem::CS_Success);
